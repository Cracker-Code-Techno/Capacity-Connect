          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div className="lg:col-span-1 space-y-6">
              <div className="glass-panel p-6 rounded-2xl border border-[rgba(255,255,255,0.05)]">
                <h2 className="font-bold text-lg mb-4 text-white">Course Content</h2>
                <div className="space-y-2">
                  {course.modules?.map((mod: any, index: number) => (
                    <button
                      key={mod.id}
                      onClick={() => setActiveModuleId(mod.id)}
                      className={`w-full text-left p-3 rounded-lg flex items-center gap-3 transition-colors ${activeModuleId === mod.id ? 'bg-[#a855f7]/20 text-[#a855f7] border border-[#a855f7]/30' : 'hover:bg-white/5 text-gray-400 border border-transparent'}`}
                    >
            ...
            <div className="lg:col-span-3">
              <div className="glass-panel p-8 md:p-12 rounded-2xl border border-[rgba(255,255,255,0.05)] min-h-[600px]">
                {activeModule ? (
                  <>
                    <h1 className="text-3xl font-extrabold text-white mb-8 pb-4 border-b border-white/10">{activeModule.title}</h1>
